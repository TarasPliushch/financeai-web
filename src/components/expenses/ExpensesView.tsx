    ...
    <div className="max-w-6xl mx-auto space-y-6 animate-fade-in">
      <div className="flex justify-between items-center flex-wrap gap-2">
        ...
        <div className="flex gap-2">
          <button onClick={() => setShowCharts(true)} className="px-4 py-2 rounded-xl border border-border hover:bg-secondary transition-colors">📊 Діаграми</button>
      ...
      <div className="flex gap-2 border-b border-border"><button onClick={() => setSelectedTab('expenses')} className={`px-4 py-2 font-medium transition-colors ${selectedTab === 'expenses' ? 'text-primary border-b-2 border-primary' : 'text-muted-foreground'}`}>📉 Витрати</button><button onClick={() => setSelectedTab('income')} className={`px-4 py-2 font-medium transition-colors ${selectedTab === 'income' ? 'text-primary border-b-2 border-primary' : 'text-muted-foreground'}`}>📈 Доходи</button></div>
      
      <div className="space-y-3">
        {displayedExpenses.length === 0 ? (<div className="text-center py-12"><div className="text-6xl mb-4">📭</div><p className="text-muted-foreground">Немає записів</p><p className="text-sm text-muted-foreground mt-1">Натисніть + щоб додати</p></div>) : (displayedExpenses.map(e => (<div key={e.id} className="flex items-center gap-4 p-4 rounded-xl border border-border bg-card hover:bg-secondary/30 transition-all group"><div className="w-12 h-12 rounded-full bg-primary/10 flex items-center justify-center text-2xl">{categoryEmoji(e.category)}</div><div className="flex-1"><h3 className="font-medium">{e.title}</h3><p className="text-sm text-muted-foreground">{e.category}</p><p className="text-xs text-muted-foreground">{format(e.date, 'dd MMM yyyy', { locale: uk })}</p></div><div className="text-right"><p className={`font-semibold ${e.isIncome ? 'text-green-500' : 'text-red-500'}`}>{e.isIncome ? '+' : '-'}{currency}{e.amount.toFixed(0)}</p></div><div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity"><button onClick={() => handleEdit(e)} className="p-2 rounded-lg hover:bg-white/10 transition-colors" title="Редагувати"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M17 3l4 4-7 7H10v-4l7-7z"/><path d="M4 20h16"/></svg></button><button onClick={() => handleDelete(e)} className="p-2 rounded-lg hover:bg-red-500/20 transition-colors text-red-400" title="Видалити"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M4 7h16"/><path d="M10 11v6"/><path d="M14 11v6"/><path d="M5 7l1 12a2 2 0 0 0 2 2h8a2 2 0 0 0 2-2l1-12"/><path d="M9 3h6"/></svg></button></div></div>)))}
      </div>
