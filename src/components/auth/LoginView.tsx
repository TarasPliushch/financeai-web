...
export const LoginView: React.FC = () => {
  ...
  useEffect(() => {
    if (pendingTwoFactorEmail) navigate('/2fa', { state: { email: pendingTwoFactorEmail } });
    ...
    <div className="min-h-screen flex items-center justify-center p-4 bg-gradient-to-br from-background via-background to-primary/5">
      <div className="w-full max-w-md animate-fade-in">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button type="submit" disabled={isLoading} className="w-full py-3 rounded-xl bg-gradient-to-r from-pink-500 to-purple-500 text-white font-semibold hover:opacity-90 disabled:opacity-50">{isLoading ? 'Вхід...' : 'Увійти'}</button>
        </form>
        <div className="mt-6 text-center space-y-2">
          <Link to="/forgot-password" className="text-sm text-primary hover:underline">Забули пароль?</Link>
          <div><Link to="/register" className="text-sm text-primary hover:underline">Немає акаунту? Зареєструватися</Link></div>
        </div>
      </div>
    </div>
  );
